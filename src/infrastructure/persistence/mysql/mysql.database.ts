...
export class MysqlDatabase implements IDatabaseModel {
    ...
    private constructor(){
        this._db = mySqlConfig.database!;
        this._username = mySqlConfig.username!;
        this._password = mySqlConfig.password!;
        this._host = mySqlConfig.host!;
        this._dialect = 'mysql';
        this._port = Number(mySqlConfig.port);
    ...
    createModelReference(name: string, properties: Sequelize.ModelAttributes): Sequelize.ModelCtor<Sequelize.Model<any, any>> {
        return this._adapter.define(
            name,
            properties,
            {
